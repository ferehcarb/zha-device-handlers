...
@pytest.mark.parametrize("quirk", (zhaquirks.tuya.ts1201.ZosungIRBlaster,))
async def test_ts1201_ir_blaster(zigpy_device_from_quirk, quirk):
    """Test tuya TS1201 IR blaster."""

    irCode_to_learn = "A/AESQFAAwUIAvAErwHgAQNADwXwBK8BrwFABcADAXYfwAkBrwHACeABBwXwBK8BrwFABcAD4GgvAgRJAQ==" # a learned code 
    irCode_to_send = "B3wPfA/5AcoH4AUDAeUDgAPAC+AHB+AHA+ADN+ALBw==" # my TV poweroff/on code
    ...
    with mock.patch.object(
        ...
        print(f'hdr: {hdr} , args: {args}')
        rsp = ts1201_transmit_cluster.handle_message(hdr, args)
        await wait_for_zigpy_tasks()
        m1.assert_called_with(
            transmit_clusterid,
            3,
        ...
        print (f"listener cmds: {ts1201_transmit_listener.cluster_commands}")
        print (f"listener cmds[0][2]: {type(ts1201_transmit_listener.cluster_commands[0][2]).__name__}")
        print (f"listener cmds[0][2].length: {ts1201_transmit_listener.cluster_commands[0][2].length}")
        print (f"rsp: {rsp}")
        ...
        print (f"listener cmds: {ts1201_transmit_listener.cluster_commands}")
        print (f"listener cmds[1][2]: {type(ts1201_transmit_listener.cluster_commands[1][2]).__name__}")
        print (f"rsp: {rsp}")
...
#        Actual: request(60672, 5, b'\x01\x05\x04\x00\x01\x00\x00\x00', expect_reply=False, command_id=4)

        print (f"listener cmds: {ts1201_transmit_listener.cluster_commands}")
        print (f"listener cmds[2][2]: {type(ts1201_transmit_listener.cluster_commands[2][2]).__name__}")
        print (f"rsp: {rsp}")
#        Decoded ZCL frame: ZosungIRTransmit:resp_ir_frame_03(zero=0, seq=1, position=55, msgpart=b'h/\x02\x04I\x01', msgpartcrc=231)
...
#        Actual: request(57348, 6, b'\x01\x06\x00{"study":1}', expect_reply=True, command_id=0)

        print (f"listener cmds: {ts1201_transmit_listener.cluster_commands}")
        print (f"listener cmds[3][2]: {type(ts1201_transmit_listener.cluster_commands[3][2]).__name__}")
        print (f"rsp: {rsp}")
#        Decoded ZCL frame: ZosungIRTransmit:resp_ir_frame_03(zero=0, seq=1, position=55, msgpart=b'h/\x02\x04I\x01', msgpartcrc=231)
        ...
        # IR Send tests
        rsp = await ts1201_control_cluster.command(0x0002, code=irCode_to_send)
        await wait_for_zigpy_tasks()
        # IRSend must call irtransmit command id 0x00
        m1.assert_called_with(
            transmit_clusterid,
            7,
        ...
        rsp = ts1201_transmit_cluster.handle_message(hdr, args)
        await wait_for_zigpy_tasks()
#        \x01\t\x02\x01\x00\x00\x00\x00\x008
        m1.assert_called_with(
            transmit_clusterid,
            9,
        ...
        print(f"rsp: {rsp}")
        print (f"listener cmds: {ts1201_transmit_listener.cluster_commands}")
        #receive_ir_frame_00(seq=1, length=122, unk1=0, clusterid=57348, unk2=1, cmd=2, unk3=0))
        ...
        rsp = ts1201_transmit_cluster.handle_message(hdr, args)
        await wait_for_zigpy_tasks()
        print (f"listener transmit cmds: {ts1201_transmit_listener.cluster_commands}")
        #assert rsp == foundation.Status.SUCCESS
        print("rsp: ", rsp)
        ...
        #Actual: request(57348, 12, b'\x01\x0c\x00\x00\x01\x02\x03\x04', expect_reply=True, command_id=0)
        await wait_for_zigpy_tasks()
        # 
        m1.assert_called_with(
            control_clusterid,
